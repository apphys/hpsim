...
void SimulateBuncherKernel(double* r_x, double* r_y, double* r_phi, 
  ...
  BeamLineElement* r_elem, uint r_elem_indx)
{
  BeamLineElement buncher = *r_elem;
  uint index = blockIdx.x*blockDim.x+threadIdx.x;
  uint stride = blockDim.x*gridDim.x;
  while(index < d_const.num_particle)
  {
    if(r_loss[index] == 0)
    {
      ...
      double w = r_w[index];
      double r2 = x*x + y*y;
      double aper2 = buncher.aperture1 * buncher.aperture1;
      // check for loss
      ...
      double h = buncher.freq*d_wavelen/CLIGHT;
      ...
      q = q > 0.0 ? q : -q;
      double con1 = TWOPI*h*buncher.rf_amp*q/(beta*d_wavelen*d_const.mass);
      double arg = k2*r2*0.25;
      double ph = h*phi + buncher.phi_c;
      double dw = q*buncher.rf_amp*cos(ph)*(1.0+arg*(1.0+arg*(0.25+arg/36.0)));
...
/*! The reason Dipole was not broken up into left edge + half sector + half sector + right edge is to copy data once (x,y,xp,yp), do as many calc as it can. Breaking into smaller pieces needs more copying*/
__global__
void SimulateFirstHalfDipoleKernel(double* r_x, double* r_y, double* r_phi, 
  double* r_xp,double* r_yp, double* r_w, uint* r_loss, BeamLineElement* r_elem)
{
//  __shared__ BeamLineElement dipole;
//  if(threadIdx.x == 0) dipole = *r_elem;
//  __syncthreads();
  
  BeamLineElement dipole = *r_elem;
  uint index = blockIdx.x*blockDim.x+threadIdx.x;
  uint stride = blockDim.x*gridDim.x;
  ...
  while(index < d_const.num_particle)
  {
    if(r_loss[index] == 0)
    {
      ...
      double h = 1.0/dipole.length;
      double b1 = dipole.tp;
      double hf_g = dipole.dg;
      double k1 = dipole.phi_c;
      double k2 = dipole.phi_out;
      double findx = dipole.t;
      double design_w = dipole.energy_out;
      ...
      double h2 = h*h;
      double dl = 0.5*dipole.length*dipole.gradient;
      double yk2 = findx*h2;
...
void SimulateHalfQuadKernel(double* r_x, double* r_y, double* r_phi, double* r_xp, 
        double* r_yp, double* r_w, uint* r_loss, double r_length, double r_aper, 
        double r_gradient, uint r_elem_indx)
{
  double aper2 = r_aper * r_aper;
  uint index = blockIdx.x*blockDim.x+threadIdx.x;
  uint stride = blockDim.x*gridDim.x;
...
void SimulateRFGapFirstHalfKernel(double* r_x, double* r_y, double* r_phi, 
                            ...
                            double r_phi_in, BeamLineElement* r_elem, 
                            double r_qlen1 = 0.0, double r_qlen2 = 0.0,
                            bool flag_ccl = true)
{
  BeamLineElement gap = *r_elem;
  uint index = blockIdx.x*blockDim.x+threadIdx.x;
  uint stride = blockDim.x*gridDim.x;
  while(index < d_const.num_particle)
  {
    if(r_loss[index] == 0)
    {
      ...
      double phi = r_phi[index];
      double w = r_w[index];
      double cell_len = gap.length + r_qlen1*0.5 + r_qlen2*0.5;
      double dd1 = 0.5*cell_len - gap.dg - 0.5*r_qlen1; 
      r_x[index] = x + dd1 * xp;
      r_y[index] = y + dd1 * yp;
      double wave_len = CLIGHT/gap.freq;
      d_wavelen = wave_len;
      if(gap.rf_amp == 0.0)
      {
        double gm1 = w/d_const.mass;
      ...
      else
      {
        ...
        if(!flag_ccl) // dtl
        {
          double dps = gap.phi_c - r_phi_in;
          double d_cell_len = cell_len - gap.beta_g*wave_len;
...
//          r_phi[index] = phi - ((beta-beta_in)*(PI*gap.cell_len_bl+dps)/beta-dps) + dphi_clen;
          r_phi[index] = phi + (1.0 - (beta-beta_in)/beta)*
            (PI*gap.cell_len_bl + dps) + dphi_clen - dphi_qlen1;
        }
        else // ccl
        {
          double dps = gap.phi_c - r_phi_in;
          double betag = cell_len/(gap.cell_len_bl*wave_len);
          r_phi[index] = phi + (1.0 - (beta-beta_in)*betag/(beta*beta_in))*
                (PI*gap.cell_len_bl + dps);
        }
      }
    }// if loss
    index += stride;
...
//__device__
//void GetTTP(double* t, double* tp, double* sp, double beta_design, double beta)
//{
//  double t0 = *t;
//  double bratio = beta/beta_design - 1; 
////  double tratio = 1.008 + 0.4399 * bratio + 0.061 * bratio * bratio + 2.0551 * bratio * bratio * bratio;  
////  *t = tratio * t0;
////  *tp = (0.4399 + 0.061 * 2.0 * bratio + 2.0551 * 3.0 * bratio * bratio) * (t0 / TWOPI)*beta*beta/(beta_design*beta_design);
//  double tratio = 1.004 + 0.3884 * bratio - 0.0755 * bratio * bratio + 1.7563 * bratio * bratio * bratio;  
//  *t = tratio * t0;
//  *tp = (0.3884 - 0.0755 * 2.0 * bratio + 1.7563 * 3.0 * bratio * bratio) * (t0 / TWOPI)*beta*beta/(beta_design*beta_design);
//
//}
                            ...
                            bool flag_ccl = true)
{
  ...
  { 
    if(r_loss[index] == 0)
    {
      ...
      GetTransitTimeFactors(&t_fac, &tp_fac, &sp_fac, beta, gap.sf_betag, gap.sf_betamin, 
        gap.sf_t_a0, gap.sf_t_a1, gap.sf_t_a2, gap.sf_t_a3, gap.sf_t_a4, gap.sf_t_a5, 
        gap.sf_s_a1, gap.sf_s_a2, gap.sf_s_a3, gap.sf_s_a4, gap.sf_s_a5);

//      if(index == 0)
//        printf("coeffs: %f, %f, %f, %f, %f, %f, %f\n", gap.sf_t_a0, gap.sf_t_a1, gap.sf_t_a2, gap.sf_t_a3, gap.sf_s_a1, gap.sf_s_a2, gap.sf_s_a3);
//        printf("transit factors, t [%f, %f], tp [%f, %f], sp [%f, %f]\n", gap.t, t_fac, gap.tp, tp_fac, gap.sp, sp_fac);
      if(gap.rf_amp == 0.0 || (flag_ccl && beta < gap.sf_betamin)) //(!flag_ccl && beta < 0.035) || 
      {
        r_x[index] = x + dd2*xp;
        r_y[index] = y + dd2*yp;
        double gm1 = r_w[index]/d_const.mass;
      ...
      {
        double wave_len = CLIGHT/gap.freq;
        double cl = cell_len/gap.cell_len_bl;
        double betag_ccl = cl/wave_len;
        double betag;
        if(flag_ccl) // ccl
          betag = gap.beta_g;// beta at the center of the gap
        else // dtl
          betag = betag_ccl;
        ...
        double efl = gap.rf_amp*cell_len;
...
//          tw = t_fac-TWOPI*tp_fac*(beta_in/beta-1.0);
    ...
    //*/ 
        ...
        double sinps, cosps;
        sincos(gap.phi_c, &sinps, &cosps);
        double phi1, sinp1, cosp1;
        if(flag_ccl) // ccl
          phi1 = phi + gap.phi_out - TWOPI*gap.dg/(beta*wave_len) + r_ccl_cell_num * PI;
        else // dtl
          phi1 = phi + gap.phi_out;
    ...
    //*/
        ...
        if(flag_ccl) // ccl
        {
          double dps = r_phi_in - gap.phi_c;
          phi_new = phi + (1.0 - (beta-beta_out)*betag_ccl/(beta*beta_out))*
                         (PI*gap.cell_len_bl + dps) + prome; // absolute phase
        }
        else // dtl
        {
          double d_cell_len = cell_len - gap.beta_g*wave_len;
          ...
          if(d_cell_len > 1e-15 || d_cell_len < -1e-15)
            dphi_clen = PI*d_cell_len/(beta*wave_len);
          double dps = gap.phi_c - r_phi_in;
//          r_phi[index] = phi-(beta-beta_out)/beta*(PI*gap.cell_len_bl+dps)+prome+dps+dphi_clen;
          phi_new = phi + (1.0 -(beta-beta_out)/beta)*(PI*gap.cell_len_bl + dps) +
