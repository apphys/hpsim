...
#
    ...
    def get_fit_sf(self):
        """Return factor used to scale HPSim beam intensity results
    ...
    def get_fit_parms(self):
        ...
        stored in ps201 object.

        Returns:
           fit_parms(list of lists): contains [[names, field, value] ...]
        """
        return self.fit_parms

    def get_db_fit_parms(self):
        """Get fit parameters values from model_db and return in list.
...
#            print name, field, val
    ...
    def set_fit_parms(self, parms):
        """Store user supplied fit parameters in local list.

        Arguments:
           parms([name, field, val]...]: list of fit parameters
        ...
        for elem in loc_parms:
            name, field, val = elem
            for i,elist in enumerate(self.fit_parms):
                if name == elist[0] and field == elist[1]:
#                    print name, field, val
                    self.fit_parms[i][2] = val

    def set_db_fit_parms(self, parms):
        """Set fit parameter values user supplied parms and store in local list
...
#            print name, field, val
    ...
    def get_phs_fit_parm(self):
        """Return name, field, val of phase_offset fit parameter(s)
        stored in ps201 object.
        May return more than one group.

        Returns:
           [[name, field, val],...] of phase offset fit parameter(s)
           or
    ...
    def set_fit_parm_vals(self, pval):
        """Store list of bare parameter values in ps201 objects fit_parms list.
        """
        for i, elem in enumerate(self.fit_parms):
            self.fit_parms[i][2]=pval[i]

    def store_fit_parms(self, parms):
        """Update model_db fit parameters (cal const) with user supplied values.
...
#            print name, field, val
    ...
    def update_fit_parms(self):
        """Update model_db with latest fit parmeter values taken 
        from ps201 object fit_parms.
    ...
    def quick_ps_fit(self, beam, sim, xmeas, ymeas, fig=None):
        """Determine fit parameters: sim_scale_factor, phase_offset & amp_scl_fac
        using leading edge and fwhm info from simulation results and measurements.
        Much faster than optimize_ps_fit routine.

        Arguments:
           beam (Beam object): beam object to use in fitting ps201 data
           sim (Simulation object): Simulation object to use in fitting
           xmeas (Numpy vector): Contains ps201 phase values to simulate for
           ymeas (Numpy vector): Contains corresponding ps201 beam intensity data
           fig (PS201Plot object; optional): PS201Plot object for plotting itermediate results
        ...
        key = self.cavity
        if key=='T1' and self.rf['PB']==lcs.rf_on_val('PB') and self.rf['PB']==lcs.rf_on_val('PB'):
            print "Quick_ps_fit will not work when both PB and MB in-time"
            return [None, None, None]

        # get amp-sf and phs_off parms
        # name, field, val
        # print self.get_phs_fit_parm()
        phs_parm_name, phs_parm_field, phs_parm_init = self.get_phs_fit_parm()
        amp_parm_name, amp_parm_field, amp_parm_init = self.get_amp_fit_parm()

        parms = [self.get_fit_sf()] + self.get_fit_parm_vals()
        print '*** Starting PS201 Quick Fitting Procedure ***'
        print "Starting Fit parameters: "
        print "Fit scale factor:", self.get_fit_sf()
        print "Phase offset name, field, value:", phs_parm_name, phs_parm_field, phs_parm_init
        print "Amp Scale Factor name, field, value:", amp_parm_name, amp_parm_field, amp_parm_init
        ...
        if self.hps_intermediate[key] is None:
            ...
            print " then iterate from", hps.get_next_element(self.hps_intermediate[key]), "to ", self.hps_stop[key]

        #initilize intermediate results to nan
        fwhm_s = float('nan')
        le_s = float('nan')

        # extract values from measured data
#        xymeas = self.get_data()
#        ymeas = npu.get_y(xymeas)
        xymeas = npu.get_xy(xmeas, ymeas)
#
        fwhm_m = get_fwhm(xymeas)
        le_m = get_le(xymeas)
        max_phs_m = get_phs_of_max(xymeas)

        phs_off_new = phs_parm_init
        amp_sf_new = amp_parm_init
        const = 1.0

        # perform coase step sim to set phs offset and align left edge of sim with meas distr
        print "*** Step 1: Initial course scan to get phs offset close. ***"
        xsim = np.arange(max_phs_m - 180.0, max_phs_m +180., 30.)
        while math.isnan(fwhm_s):
            # perform simulation
            ysim = self.simulate_ps(beam, sim, xsim)

            # plot 
            if fig is not None:
                # update local copy of fit parms for fig title
                self.set_fit_sf(const)
                self.set_fit_parms([phs_parm_name, phs_parm_field, phs_off_new])
                self.set_fit_parms([amp_parm_name, amp_parm_field, amp_sf_new])
                fig.plot_ps201(self, xmeas, ymeas, xsim, ysim)

            # determine fit scale factor
            if max(ysim) > 0:
                const = self.get_fit_sf() * max(ymeas)/max(ysim)
                print "#const:", const
                self.set_fit_sf(const)

            xysim = npu.get_xy(xsim, ysim)
            fwhm_s = get_fwhm(xysim)
            le_s = get_le(xysim)
            max_phs_s = get_phs_of_max(xysim)
            print "Sim: le, fwhm, phs(max)",le_s, fwhm_s, max_phs_s

            phs_off = hps.get_db_model(phs_parm_name, phs_parm_field)
            if math.isnan(fwhm_s):
                # update using centroid
                phs_off_new = max_phs_m - max_phs_s + phs_off
            else:
                # update
                phs_off_new = le_m - le_s + phs_off
                
            # update phase_offset in model_db and ps201 object
            #hps.set_db_model(phs_parm_name, phs_parm_field, phs_off_new)
            self.set_db_fit_parms([phs_parm_name, phs_parm_field, phs_off_new])
        ...
        # now update amp sf
        print "\n*** Step 2: Update amp sf and phs offset and plot sf. ***"
        ONEOFN = 1
        xsim = xmeas[::ONEOFN] #list(xmeas)[::ONEOFN] 
        asf_res = []
        fwhm_res = []
        niter = 0
        MAX_ITER = 10
        FWHM_ERR = 0.3
        LE_ERR = 0.3
        while (niter < MAX_ITER) and ((abs(fwhm_m - fwhm_s) > FWHM_ERR) \
                                 or (abs(le_m - le_s) > LE_ERR)):
            niter+= 1
            # perform simulation
            ysim = self.simulate_ps(beam, sim, xsim)
            # plot results
            ...
            xysim = npu.get_xy(xsim, ysim)
            fwhm_s = get_fwhm(xysim)
            # append sim fwhm to results list
            fwhm_res.append(fwhm_s)

            # adjust leading edge using phase_offset
            le_s = get_le(xysim)
            print "le_m, le_s ", le_m, le_s
            phs_off = hps.get_db_model(phs_parm_name, phs_parm_field)
            phs_off_new = le_m - le_s + phs_off
            self.set_db_fit_parms([phs_parm_name, phs_parm_field, phs_off_new])
            self.set_fit_parms([[phs_parm_name, phs_parm_field, phs_off_new]])

            # adjust fwhm using amp scale factor
            if amp_parm_name is not None:
                asf_s = hps.get_db_model(amp_parm_name, amp_parm_field)
                asf_res.append(asf_s)
                if len(fwhm_res) < 2:
                    if fwhm_s > fwhm_m:
                        amp_sf_new = 0.98 * asf_s
                    else:
                        amp_sf_new = 1.025 * asf_s

                elif len(fwhm_res) == 2:
                    fit_coef = np.polyfit(fwhm_res, asf_res, 1)
                    fit_fun = np.poly1d(fit_coef)
                    amp_sf_new = fit_fun(fwhm_m)

                elif len(fwhm_res) > 2:
                    fit_coef = np.polyfit(fwhm_res, asf_res, 2)
                    fit_fun = np.poly1d(fit_coef)
                    amp_sf_new = fit_fun(fwhm_m)

                print "fwhm_m ",fwhm_m
                print "fwhm_res, asf_res"
                for item in zip(fwhm_res, asf_res):
                    print item
                print "asf_s, asf_new"
                print asf_s, amp_sf_new

                hps.set_db_model(amp_parm_name, amp_parm_field, amp_sf_new)
                #self.set_fit_parms([[amp_parm_name, amp_parm_field, asf_new]])
    ...
    #        ps.update_fit_parms()
            ...
            print "LE_m, LE_s:", le_m, le_s
            print "FWHM_m, FWHM_s:", fwhm_m, fwhm_s
            # determine plot scaling factor and update in ps201 object
            if fig is not None:
                self.set_fit_parms([phs_parm_name, phs_parm_field, phs_off_new])
                self.set_fit_parms([amp_parm_name, amp_parm_field, amp_sf_new])
                fig.plot_ps201(self, xmeas, ymeas, xsim, ysim)

            const = self.get_fit_sf()*sum( ymeas[::ONEOFN] * ysim ) / sum(ysim * ysim)
            self.set_fit_sf(const)
#            print const
#            print asf, po", amp_sf_new, phs_off_new

        self.init_fit_parms()
        self.fit_sf = const
        return self.get_fit_parms()
...
#                beam_current = self.get_fit_sf() * self.get_beam_above_ethreshold(beam)
                beam_current = fit_sf * self.get_beam_above_ethreshold(beam)
...
#            beam_current = self.get_fit_sf() * self.get_beam_above_ethreshold(beam)
    ...
    def display_info(self):
        """Print PS201 data file information, excluding scan data."""
    ...
    def get_le(self):
        """Determine the phase of half-max point of Leading Edge of PS201 distribtion."""
        xy = self.get_data()
        phslo = npu.get_halfmax_x(xy, 'lower')
        return phslo
...
class PS201Plot():
    """A class for creating PS201 plots"""
    ps201plot = ""
    ...
    def plot_ps201(self, pso, xmeas, ymeas, xsim=None, ysim=None):
        plt.clf()
        plt.title(pso.title_fit_parms())
        if xsim is None:
            plt.plot(xmeas, ymeas, 'o')
            plt.legend(['Meas'])
        else:    
            plt.plot(xmeas, ymeas, 'o', xsim, ysim, 'rx-')
            plt.legend(['Meas', 'Sim'])

        plt.ylim(ymin=0.0)
        plt.xlabel(lcs.get_pv_psp(pso.cavity))
        plt.draw()
...
def depth(l):
    """ Returns depth of Python list. If numpy array, returns ndim"""
