...
#
    ...
    def set_fit_parms(self, parms):
        """Set fit parameters values user supplied parms and store in local list"""
        for elem in parms:
            name, field, val = elem
...
#
        ...
        phs_off_init = hps.get_db_model(phs_parm_name, phs_parm_field)
        const = 1.0

        # perform coase step sim to to adjust phs offset and align sim and meas distr
        print "P1: Initial course scan to get phs offset close"
        xsim = np.arange(max_phs_m - 180.0, max_phs_m +180., 30.)
        while math.isnan(fwhm_s):
            ysim = self.simulate_ps(beam, sim, xsim)
            if max(ysim) > 0:
                const = max(ymeas)/max(ysim)
                print "#const:", const

            if fig is not None:
                fig.plot_ps201(self, xmeas, ymeas, xsim, const*ysim)

            xysim = npu.get_xy(xsim, ysim)
            fwhm_s = get_fwhm(xysim)
            le_s = get_le(xysim)
            print "le, fwhm",le_s, fwhm_s
        ...
        # now update amp sf
        print "\nWill now update amp sf and phs offset and plot sf."
        ONEOFN = 1
        xsim = xmeas[::ONEOFN] #list(xmeas)[::ONEOFN] 
        asf_res = []
        fwhm_res = []
        niter = 0
        while (niter < 5) and (abs(fwhm_m - fwhm_s) > 0.5):
            niter+= 1
            ysim = self.simulate_ps(beam, sim, xsim)
            if fig is not None:
                fig.plot_ps201(self, xmeas, ymeas, xsim, const*ysim)
            ...
            if amp_parm_name is not None:
                ...
                hps.set_db_model(amp_parm_name, amp_parm_field, asf_new)
                #self.set_fit_parms([[amp_parm_name, amp_parm_field, asf_new]])

    #        ps.update_fit_parms()
            ysim = self.simulate_ps(beam, sim, xsim)
            const = sum( ymeas[::ONEOFN] * ysim ) / sum(ysim * ysim)
            print const
            print "asf, po", asf_new, phs_off_new
