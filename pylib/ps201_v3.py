...
class PS201():
    ...
    def __init__(self):
        ...
        Attributes:
           ...
           pkcur(float): peak current(mA)
           gatelen(int): beam gate length
           reprate(int): repetition rate(Hz)
           bgcur(float): background current(mA)
           actuator(list, string): Actuator status
           target(float): Nominal Phase of Tank being scanned (used to calc DCLE)
           cavsp(list of list of floats): Cavity set points
           npts(int): number of scan points
           data(list of pairs of floats): Numpy array of phase & collector current pairs
          Additional quantities in class (useful for fitting and simulating)
           plimits(list of floats): phase limits for analysis purposes
           threshold(float): threshold applied to collector current for analysis purposes
           fit_sf(float): overall scale factor applied to simulated beam current to match measurement
...
#        print phslo, phscent, phsup
    ...
    def display_results(self):
        ...
        If xysim data present, include in analysis
        """
...
def get_le(xypairs):
    """Return leading edge of ps201 distribution from xypairs"""
...
def get_phs_of_max(xypairs):
    """Returns phase (x) of maximum in y"""
