...
import sys
import os
_PAR_DIR = os.path.abspath(os.path.pardir)
...
# functions from original HPSim.so 
    ...
    def get_distribution(self, option='all'):
        ...
        """
        loss_all = np.array(self.get_losses())
        if option == 'all':
            loss = loss_all
        elif option == 'bad':
            ...
            loss = np.where(loss_all == 0)[0]
        return np.array(self.get_x(option) * _CM), \
            np.array(self.get_xp(option) * _MR), \
            np.array(self.get_y(option) * _CM), \
            np.array(self.get_yp(option) * _MR), \
            np.array(self.get_phi(option) * _DEG), \
            np.array(self.get_w(option)), \
            np.array(loss)
    ...
    def get_x(self, option = 'all'):
        """Return x coordinates (cm) of macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_x(option)) * _CM

    def get_xp(self, option = 'all'):
        """Return xp coordinates (mr) of macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_xp(option)) * _MR

    def get_y(self, option = 'all'):
        """Return y coordinates of (cm) macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_y(option)) * _CM

    def get_yp(self, option = 'all'):
        """Return yp coordinates of (mr) macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_yp(option)) * _MR

    def get_phi(self, option = 'all'):
        """Return phi coordinates (deg) of macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_phi(option)) * _DEG

    def get_w(self, option = 'all'):
        """Return w coordinates (MeV) of macro particles, option = 'good', 
        'bad', 'all (default)"""
        return np.array(self.beam.get_w(option))

    def get_losses(self):
        """Return loss condition of macro particles"""
        return np.array(self.beam.get_losses())
...
################################ new functions #################################
    ...
    def get_coor(self, var, mask = None):
        ...
        if lc_var in _COORDINATES:
            request += "('all')"
            if mask is not None:
                coor = np.array(eval(request))[mask] * _USER_UNITS[lc_var]
            else:
                coor = np.array(eval(request)) * _USER_UNITS[lc_var]
            if lc_var == 'phi':
                coor = modulo_phase(coor, self.get_ref_phi())
        elif lc_var in _LOSSES:
            request += "()"            
            if mask is not None:
                coor = np.array(eval(request))[mask]
            else:
                coor = np.array(eval(request))
        else:
    ...
    def get_mask_with_limits(self, var, lolim, uplim = None):
        ...
        lc_var = var.lower()
        request = "self.beam.get_" + str(lc_var)
        if lc_var in _COORDINATES:
            request += "('all')"
        elif lc_var in _LOSSES:
            request += "()"
    ...
    def get_good_mask(self, mask = None):
        ...
        if mask is not None:
            return np.intersect1d(mask, np.where(np.array(self.beam.get_losses()) == 0)[0])
        else:
            return np.where(np.array(self.beam.get_losses()) == 0)[0]
...
class Distribution():
    ...
    def get_twiss(self, var):
        """Return Twiss parameters (a,b,eurms) for specified coords x, y or z"""
        lc_var = var.lower()
        if lc_var in _EMITTANCE:
            v = lc_var
            vp = v + 'p'
...
def modulo_phase(phase_dist, ref_phs):
    """Return the phase coordinates of beam after modulo 360 deg has been applied
...
def get_element_list(start_elem_name = None, end_elem_name = None, elem_type = None):
    ...
    Arguments:
       ...
       elem_type(str): type of element to retrieve
    """
    request = "HPSim.get_element_list(DBConnection.dbconnection"
    if start_elem_name:
        request += ', ' + '"' + start_elem_name + '"'
    if end_elem_name:
        request += ', ' + '"' + end_elem_name + '"'
    if elem_type:
        request += ', type=' + '"' + elem_type + '"' 
    request += ')'
#    print request
    return eval(request)
