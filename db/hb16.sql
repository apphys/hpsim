...
INSERT INTO "quad" VALUES(1, 'TRQM05', 7.0, 0, 16.341, 0.12, 0.0, 1, 0.0, 1, 0, 468, 'quad');
INSERT INTO "quad" VALUES(2, 'TRQM06', 9.0, 0, -12.689, 0.12, 0.0, 2, 0.0, -1, 0, 470, 'quad');
INSERT INTO "quad" VALUES(3, 'TRQM07', 21.0, 0, -19.648, 0.12, 0.0, 3, 0.0, -1, 0, 493, 'quad');
INSERT INTO "quad" VALUES(4, 'TRQM08', 23.0, 0, 20.7282, 0.12, 0.0, 4, 0.0, 1, 0, 495, 'quad');
...
CREATE VIEW channel_list as 
  select view_index, name, model_type, (select lcs_name from epics_channel where id = amplitude_channel) as channel1, 
    ...
    (select lcs_name from epics_channel where id = on_off_channel) as channel3 from buncher union
  select view_index, name, model_type, (select lcs_name from epics_channel where id = channel), NULL, NULL from quad union
  select view_index, name, model_type, (select lcs_name from epics_channel where id = channel), 
    (select lcs_name from epics_channel where id = channel2), NULL from drift where channel != '' union
  select view_index, name, model_type, (select lcs_name from epics_channel where id = channel), 
    (select lcs_name from epics_channel where id = channel2), NULL from raperture where channel != '' union
  select view_index, name, model_type, (select lcs_name from epics_channel where id = channel), 
    (select lcs_name from epics_channel where id = channel2), NULL from dipole;
