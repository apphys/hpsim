...
//    printf("update scheff size kernel: nr=%d, nz=%d, r^2_max=%18.16f, |z|_max=%18.16f, dr=%18.16f, dz=%18.16f, bl = %18.16f\n", 
//          d_scheff_param.nx,d_scheff_param.nz, r_partial_r[0], r_partial_z[0],
//          scheff_sz.dr, scheff_sz.dz, bl);
}
                ...
                uint r_num_part, double r_current_ov_mc2, uint r_adj_bunch)
{
  ...
  double g = 1.0-a;
  double h = log(g);
  double ee=1.0+g*(0.4630106-0.2452740*h+g*(0.1077857-0.04125321*h));
  double ek=1.38629436-0.5*h+g*(0.1119697-0.1213486*h+g*(0.07253230-0.02887472*h));
  double er1 = 0.0;
  a = sqrt(b+d2);
  ...
  for(int i = 1; i <= r_adj_bunch; ++i)
  {
    ...
    for(int j = 0; j < 2; ++j)
    {
      ...
      g = 1.0-a;
      h = log(g);
      ee=1.0+g*(0.4630106-0.2452740*h+g*(0.1077857-0.04125321*h));
      ek=1.38629436-0.5*h+g*(0.1119697-0.1213486*h+g*(0.07253230-0.02887472*h));
      a = sqrt(b+d2);
      if(rp != 0.0) 
        er1 += (ek-(rs2mrp2+d2)*ee/(c+d2))/(2.0*rp*a);
      ez1 += d1*ee/(a*(c+d2));
...
{
//  printf("Double precision atomicAdd is called\n");
  ...
  uint r_num_part, double r_beta, double r_freq)
{
  ...
  while (np < r_num_part)  
  {
    if(r_loss[np] == 0 && r_lloss[np] == 0)
    {
      ...
      int i, jm1, i1, j1, k;
      double rsq_sh = (x-xbar)*(x-xbar)/epsq+(y-ybar)*(y-ybar)*epsq;
      ...
      if(i <= d_scheff_param.nx)
      {
        ...
        if(abs(z) < hl)
        {
          ...
          i1 = i+1;
          double drdr = dr*dr;
          if(rsq_sh < 0.5*drdr*(double)(i*i+(i-1)*(i-1))) i1 = i-1;
          if(i1 < 1) i1 = 1;
  ...
  double r_ratio_q = 1, double r_ratio_gm = 1.0)
{
  ...
  uint stride = blockDim.x * gridDim.x;
//  if(indx == 0)
//    printf("Apply kick kernel: num_on_mesh = %d, adj_bunch = %d\n", num_on_mesh, r_adj_bunch);
...
//    if(r_loss[indx] == 0 && w/r_mass > (avggamma-1)*0.95) // not lost
    if(r_loss[indx] == 0 && r_lloss[indx] == 0) // not lost
    {
      ...
      double wave_len = CLIGHT/r_freq;
      double hl = d_scheff_param.nz*0.5*dzg;
      double c1 = 5721.67*r_current/(r_freq*1e6*(double)r_num_part*r_mass);
      ...
      if(r > d_scheff_param.nx*dr || abs(z) > hl) // not on mesh
      {
          ...
          double cbgcoef = c1*c3*PI*0.5*num_on_mesh; // num_on_mesh is count when get aa bin tbl
          cbgr = cbgcoef*rod3; 
          cbgzs = cbgcoef*zod3; 
      }
      else        // on mesh
      {
        ...
        {
          m = tbl_sz-1;
//          printf("!!!Error Kick beam Kernel: m out of range\n");
        }
        er_ratio = r_ratio_q / (r_ratio_r*r_ratio_z*r_ratio_gm*r_ratio_gm);
        ez_ratio = r_ratio_q / (r_ratio_z*r_ratio_z);
        cbgr = er_ratio*c3*(d*(a*r_fld_tbl[l].x+b*r_fld_tbl[l-1].x)+
                c*(a*r_fld_tbl[m].x+b*r_fld_tbl[m-1].x));
        cbgzs = ez_ratio*c3*(d*(a*r_fld_tbl[l].y+b*r_fld_tbl[l-1].y)+
                c*(a*r_fld_tbl[m].y+b*r_fld_tbl[m-1].y));
      }
      bgx += cbgr*xovr;
      bgy += cbgr*yovr;
      bgzstar += cbgzs;
      ...
      if (bgzf <= 0.0)
      {
        r_loss[indx] = 9999; // lost, but don't know the element num 
      }
      else
      {
        r_xp[indx] = bgx/bgzf;
