...
void UpdateMaxR2PhiKernelCall(Beam* r_beam)
{
//  cudaEvent_t start, stop; 
  uint block_sz, grid_sz;
  uint num = r_beam->num_particle;
...
//  StartTimer(&start, &stop);
  ...
  UpdateMaxR2PhiKernel<<<1, 1>>>(r_beam->r_max, r_beam->abs_phi_max, 
                                  partial1, partial2);
...
void SetDoubleValue(double* r_pos, uint r_index, double r_val)
{
  SetValueKernel<<<1, 1>>>(r_pos, r_index, r_val);
}
...
void InitPhiAvg(Beam* r_beam)
{
  cudaMemcpy(r_beam->phi_avg_good, r_beam->phi_avg, sizeof(double), cudaMemcpyDeviceToDevice); 
}
